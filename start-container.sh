#!/bin/bash
# 容器启动脚本: 同容器拉起 Java 应用(8080) + WeCom 反向隧道桥(80) + 钉钉入口(长连接, 免域名)
# 云托管只暴露一个端口(80), 桥作为边缘:
#   - 自己处理 /callback /tunnel /internal /health
#   - 其余路径反向代理到同容器的 Java 应用 (JAVA_UPSTREAM, 默认 127.0.0.1:8080)
# 钉钉入口用 Stream 长连接主动外连钉钉收消息, 免域名; 收到后调本地/隧道 Ollama。
...
# 4) 钉钉入口 (Stream 长连接收消息, 免域名) —— 仅当配置了 DINGTALK_CLIENT_ID 才启动
DINGTALK_PID=""
if [ -n "$DINGTALK_CLIENT_ID" ] && [ -n "$DINGTALK_CLIENT_SECRET" ]; then
  echo "[start] 钉钉入口 (Stream 长连接模式, 免域名)"
  node src/dingtalk-ingest.js &
  DINGTALK_PID=$!
fi

# 转发终止信号给子进程
trap "kill -TERM $JAVA_PID $BRIDGE_SRV $BRIDGE_WRK $DINGTALK_PID 2>/dev/null" TERM INT
